...
function anps_heading_add_custom_box() { 
    ...
    foreach($screens as $screen) {
        ...
        add_meta_box('anps_heading_meta', __('Page title and breadcrumbs', 'accounting'), 'display_meta_box_heading', $screen, 'normal', $anps_priority);
    }
    add_meta_box('anps_heading_meta', __('Page title and breadcrumbs', 'accounting'), 'display_meta_box_heading', 'portfolio', 'normal', 'core');
}
...
function display_meta_box_heading( $post ) { 
    ...
    $anps_full_height = get_post_meta($post->ID, $key ='anps_full_height', $single = true );   
    $checked = '';
    if($value2=='1') {
        $checked = 'checked';
    }
   ...
   <p></p>
    ...
    <table class="page-title hideall min300">    
        <tr>
            <td> 
                <label for="heading_bg"><?php _e("Page heading background", 'accounting'); ?></label>
            </td>
            <td>
        ...
        <tr>
            <td>
                <?php _e("Full screen heading", 'accounting'); ?> 
            </td>
            <td>
...
<table class="page-title showhide hideall min300">
    <tr>
        <td>
            <label for="anps_full_color_top_bar"><?php _e("Color top bar", 'accounting'); ?></label>
        </td>
        <td>
    ...
    <tr>
        <td>
            <label for="anps_full_color_title"><?php _e("Color menu, title and breadcrumbs", 'accounting'); ?></label>
        </td>
        <td>
    ...
    <tr>
        <td>
            <label for="anps_full_hover_color"><?php _e("Hover color", 'accounting'); ?></label>
        </td>
        <td>
    ...
    <tr>
        <td>
            <label for="anps_full_height"><?php esc_html_e('Height', 'accounting'); ?></label>
        </td>
        <td>
            <input type="text" name="anps_full_height" id="anps_full_height" value="<?php echo esc_attr($anps_full_height); ?>" />px
        </td>
    </tr>
    <tr>
        <td>
...
function anps_heading_save_postdata($post_id) {
    ...
    if (!isset($_POST['anps_full_height'])) {
        $_POST['anps_full_height'] = '';
    }
    $anps_full_height = $_POST['anps_full_height'];
    ...
    add_post_meta($post_ID, 'anps_full_height', $anps_full_height, true) or update_post_meta($post_ID, 'anps_full_height', $anps_full_height);
}
