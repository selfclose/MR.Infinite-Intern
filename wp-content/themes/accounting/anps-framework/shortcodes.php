...
function anps_recent_blog_func($atts, $content) {
	...
	'post_status'      => 'publish');
     ...
     $recent_post_text ='';
     if($posts->have_posts()) :
        $recent_post_text .= '<div class="recent-posts clearfix">';
        while($posts->have_posts()) : 
            $posts->the_post();  
            global $more;
            $more = 0;
            ...
            if(get_the_post_thumbnail(get_the_ID())!='') {
                ...
                $recent_post_text .=  '<div><i class="fa fa-search"></i> <span class="sr-only">'.get_the_title().'</span></div>';
            ...
            $recent_post_text .= "<span>".get_the_date("j.m.Y")."</span> / by "."<span>". get_the_author(). "</span> / <span>".get_comments_number()." ".__("comments", 'accounting')."</span>";
            $recent_post_text .= "</div>";
...
function icon_func( $atts,  $content ) {
    ...
    return '<div class="icon '.$class.' '.$position.'">
            '.$el_pre.'
                <i class="'.$icon_type_class.'"'.$icon_c.'></i>
                <h2'.$title_c.'>'.$title.'</h2>
