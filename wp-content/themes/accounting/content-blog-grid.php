...
if(anps_header_media(get_the_ID(), "blog-grid")!="") {
    ...
    $post_data .= "<article class='post style-3$sticky_class' id='site-content'>";
	$post_data .= "<header>";
    ...
    if( get_option('anps_post_meta_comments') != 'on' ) {
        ...
        $post_data .= "<span class='post-meta-comments'>".get_comments_number()." "._n('comment', 'comments', get_comments_number(), 'accounting')."</span>";
    }
    $post_data .= "</header>";
    ...
    if( get_option('anps_post_meta_comments') != 'on' ) {
        ...
        $post_data .= "<span class='post-meta-comments'>".get_comments_number()." ".__("comments", 'accounting')."</span>";
    }	$post_data .= "</header>";
    $post_data .= "<div class='post-content'>".$content_text."</div>"; 
    $post_data .= "</div>";
}
