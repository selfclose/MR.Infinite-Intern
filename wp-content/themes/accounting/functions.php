...
function anps_get_option($class, $value) { 
...
function anps_register_custom_fonts() { 
    ...
    if ($_GET['activated'] == 'true' && $theme->get_template() == 'accounting') { 
        include_once get_template_directory().'/anps-framework/classes/Options.php';
...
function anps_scripts_and_styles() {
    ...
    wp_enqueue_script( "moment", get_template_directory_uri()  . "/js/moment.js", '', '', true );
    wp_enqueue_script( "pikaday", get_template_directory_uri()  . "/js/pikaday.js", '', '', true );
    ...
    wp_localize_script( 'functions', 'anps', array(
        'search_placeholder' => __( 'Search...', 'accounting' )
    ));
                    ...
                    <li class="widget-container widget_nav_menu">
                        <div class="menu-main-menu-container">
                            <ul class="menu">
                                ...
                                <li class="menu-item<?php if($page == "edit_account"){ echo " current-menu-item page_item current_page_item current_page_parent"; } ?>"><a href="<?php echo wc_customer_edit_account_url(); ?>"><?php _e("Edit Account", 'constructo'); ?></a></li>
                                <?php
...
function anps_theme_styles() {     
    ...
    if (get_option('font_source_1', "Google fonts")=='Google fonts') {
        ...
        wp_enqueue_style( "font_type_1",  'https://fonts.googleapis.com/css?family=' . get_option('font_type_1', 'PT+Serif') . ':400italic,400,600,700,300&subset='.$font1_implode_subset);
    } else {
    ...
    if (get_option('font_source_navigation', "Google fonts")=='Google fonts' && get_option('font_type_1', 'PT+Serif')!=get_option('font_type_navigation', 'PT+Serif')) {
        $font3_subset = get_option("font_type_navigation_subsets", array("latin", "latin-ext")); 
        $font3_implode_subset = implode(",", $font3_subset);
        wp_enqueue_style( "font_type_navigation",  'https://fonts.googleapis.com/css?family=' . get_option('font_type_navigation', 'PT+Serif') . ':400italic,400,600,700,300&subset='.$font3_implode_subset);
    }
    
    if (get_option('anps_text_logo_source_1', "Google fonts")=='Google fonts' && get_option('font_type_1', 'PT+Serif')!=get_option('anps_text_logo_font', 'PT+Serif')) {
    ...
    wp_enqueue_style( "theme_core_style", get_template_directory_uri() . "/css/core.css" );
