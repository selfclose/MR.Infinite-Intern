...
if(is_sticky(get_the_ID())) {
    ...
    $post_data = "<article class='post style-2$sticky_class' id='site-content'>";
    $post_data .= "<header>";
    ...
    /* Post Meta */
    if( get_option('anps_post_meta_comments') != 'on' ||
        get_option('anps_post_meta_categories') != 'on' ||
        get_option('anps_post_meta_author') != 'on' ||
        get_option('anps_post_meta_date') != 'on' ) {
        $post_data .= "<div class='post-meta'>";
        $post_data .= "<ul>";

        /* Comments */
        if( get_option('anps_post_meta_comments') != 'on' ) {
            $post_data .= "<li class='post-meta-comments'><i class='hovercolor fa fa-comment-o'></i><a href='".get_permalink()."#comments'>" . get_comments_number(). ' ' . _n('comment', 'comments', get_comments_number(), 'accounting') ."</a></li>";
        }

        /* Categories */
        if( get_option('anps_post_meta_categories') != 'on' ) {
        ...
        /* Author */
        if( get_option('anps_post_meta_author') != 'on' ) {
            $post_data .= "<li class='post-meta-author'><i class='hovercolor fa fa-user'></i>".__("posted by", 'accounting')." <a href='".get_author_posts_url(get_the_author_meta('ID'))."' class='author'>".get_the_author()."</a></li>";
        }

        /* Date */
        if( get_option('anps_post_meta_date') != 'on' ) {
    ...
    $post_data .= '<a class="btn btn-sm style-4" href="'.get_permalink().'">'.__("Read more", 'accounting').'</a>';
